...
func cmdAutostart(args []string) error {
	...
	return fmt.Errorf(i18n.Tr("не понял «%s»: steno autostart on | off | status"), verb)
}
...
func autostartTarget(cfgPath string) (autostartSpec, error) {
	...
	if err != nil {
		return autostartSpec{}, fmt.Errorf(i18n.Tr("не нашёл собственный бинарник: %w"), err)
	}
	...
	if cfg, err := core.LoadConfig(p.Config); err == nil && cfg.Bot.ShutdownGrace.D() > 0 {
	...
	default:
		return autostartSpec{}, errors.New(i18n.Tr("автозапуск умею заводить только на macOS и Linux"))
	}
	return s, nil
}
...
func autostartOn(cfgPath string) error {
	...
	if _, _, state := findDaemon(cfgPath); state == daemonRunning {
		fmt.Fprintln(daemonOut, i18n.Tr("steno уже работает — передаю его службе"))
	...
	fmt.Fprintln(daemonOut, i18n.Tr("steno будет запускаться при входе в систему"))
	fmt.Fprintf(daemonOut, i18n.Tr("  служба       %s\n"), s.Unit)
	fmt.Fprintf(daemonOut, i18n.Tr("  лог          %s\n"), s.Log)
	fmt.Fprint(daemonOut, i18n.Tr("  проверить    steno status\n"))
	fmt.Fprint(daemonOut, i18n.Tr("  убрать       steno autostart off\n"))
	if s.Manager == "systemd" {
		// Без linger systemd гасит пользовательские службы при выходе из
		// сеанса: сервис, заведённый по ssh, умрёт вместе с сессией.
		fmt.Fprintln(daemonOut, dim(i18n.Tr("  чтобы работало и без входа в систему:")))
...
func autostartOff() error {
	s, err := autostartTarget(core.DefaultConfigPath)
	if err != nil {
		return err
	}
	if _, err := os.Stat(s.Unit); err != nil {
		fmt.Fprintln(daemonOut, i18n.Tr("автозапуск и так не заведён"))
		return nil
	}
	...
	fmt.Fprintln(daemonOut, i18n.Tr("steno больше не запускается при входе в систему"))
	fmt.Fprintln(daemonOut, i18n.Tr("  запустить вручную:  steno start"))
	return nil
}
...
func autostartStatus() error {
	...
	if !autostartInstalled(s) {
		fmt.Fprintln(daemonOut, i18n.Tr("автозапуск выключен"))
		fmt.Fprintln(daemonOut, i18n.Tr("  включить:  steno autostart on"))
		return nil
	}
	fmt.Fprintf(daemonOut, i18n.Tr("автозапуск включён (%s)\n"), s.Manager)
	fmt.Fprintf(daemonOut, i18n.Tr("  служба     %s\n"), s.Unit)
	fmt.Fprint(daemonOut, i18n.Tr("  выключить  steno autostart off\n"))
	return nil
}
...
func autostartLine() string {
	s, err := autostartTarget(core.DefaultConfigPath)
	if err != nil {
		return dim(i18n.Tr("не умею на этой системе"))
	}
	if autostartInstalled(s) {
		return fmt.Sprintf(i18n.Tr("включён (%s)"), s.Manager)
	}
	return i18n.Tr("выключен  ") + dim("steno autostart on")
}
...
func systemdUnit(s autostartSpec) string {
	var b strings.Builder
	b.WriteString("[Unit]\n")
	b.WriteString(i18n.Tr("Description=steno — заметки и follow-up с созвонов\n"))
