...
func (alreadyRunningError) Error() string { return i18n.Tr("steno уже работает") }
...
func rememberedDaemons() []daemonPaths {
	...
	for _, pid := range readDaemonPointer() {
		...
		out = append(out, daemonPaths{Dir: dir, PID: pid,
			Log: filepath.Join(dir, logFileName), Config: filepath.Join(dir, core.DefaultConfigPath)})
	}
	return out
}
...
func acquireDaemonLock(p daemonPaths, info daemonInfo) (*pidLock, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		if err := lockFile(f); err != nil {
			...
			if rerr == nil && other.PID > 0 {
				return nil, fmt.Errorf(i18n.Tr("%w: pid %d, запущен %s"),
	...
	return nil, fmt.Errorf(i18n.Tr("не удалось взять %s: файл подменяют на ходу"), p.PID)
}
...
func configArg(args []string) string {
	...
	return core.EnvOr("STENO_CONFIG", core.DefaultConfigPath)
}
...
func humanSize(n int64) string {
	switch {
	case n < 1024:
		return fmt.Sprintf(i18n.Tr("%d Б"), n)
	case n < 1024*1024:
		return fmt.Sprintf(i18n.Tr("%.0f КБ"), float64(n)/1024)
	case n < 1024*1024*1024:
		return fmt.Sprintf(i18n.Tr("%.1f МБ"), float64(n)/(1024*1024))
	}
	return fmt.Sprintf(i18n.Tr("%.1f ГБ"), float64(n)/(1024*1024*1024))
}
...
func sinceText(d time.Duration) string {
	...
	switch {
	case d < time.Minute:
		return fmt.Sprintf(i18n.Tr("%d сек"), int(d.Seconds()))
	case d < time.Hour:
		return fmt.Sprintf(i18n.Tr("%d мин"), int(d.Minutes()))
	case d < 24*time.Hour:
		return fmt.Sprintf(i18n.Tr("%d ч %d мин"), int(d.Hours()), int(d.Minutes())%60)
	}
	return fmt.Sprintf(i18n.Tr("%d дн %d ч"), int(d.Hours())/24, int(d.Hours())%24)
}
