...
# модели. Приходит фразой («Созвон команды разработки. Участвуют Ануар и
# Рустем. Обсуждают Сапар.»). У Groq это поле API prompt — та же initial
# prompt, что и --prompt у whisper.cpp, с тем же потолком в 224 токена. Про
# перенос подсказки из окна в окно решать не приходится: файл уходит целиком,
# и подсказку к окнам приставляет сама их сторона.
#
# Как и у whisper.cpp, подсказка чинит слова, но ломает разбивку на реплики,
# поэтому со словарём сервис зовёт адаптер дважды — с STENO_PROMPT и без — и
# склеивает проходы сам (internal/audio/merge.go). Это второй запрос к Groq
# на ту же запись; словарь того стоит, а выключается он в настройках
...
# confidence — уверенность движка в реплике. У OpenAI-совместимого ответа она
# приходит логарифмом средней вероятности токена (avg_logprob), и exp переводит
# её обратно в 0..1. Поля может не быть вовсе — тогда пишем null, то есть
# честное «не знаю»: ноль означал бы «мусор», а это разные вещи.
#
# Уверенности по словам здесь нет и взять её негде: timestamp_granularities даёт
# времена слов, но не вероятности. Поэтому этот адаптер видит только грубую
# картину — плохо расслышанная реплика целиком, — а не то самое одно слово
# посреди верной фразы, из-за которого задача уезжает не на того человека. Кому
# нужна точность до слова, тому whisper-cpp.sh или assemblyai.sh.
#
# Живьём не проверено: ключа Groq на машине нет. Форма ответа взята из
# документации OpenAI verbose_json, которой Groq следует; если avg_logprob там
# не окажется, поле просто будет null и всё останется как раньше.
jq '{segments: [.segments[] | {
      start: .start,
      end:   .end,
      text:  (.text | gsub("^\\s+|\\s+$"; "")),
      confidence: (if (.avg_logprob | type) == "number"
                   then ((.avg_logprob | exp) * 1000 | round / 1000)
                   else null end)
