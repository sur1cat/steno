...
func TestRenderMonologueХранитТаймкодКаждойСтроки(t *testing.T) {
	...
	if strings.Count(renderTranscript(segs), "[") > 1 {
...
func TestNoteAuthorБерётсяИзУчастника(t *testing.T) {
	if got := noteAuthor(&Meeting{Participants: []string{"Рустем"}}); got != "Рустем" {
		t.Errorf("автор %q", got)
	}
	if got := noteAuthor(&Meeting{Participants: []string{"  ", "Вика"}}); got != "Вика" {
		t.Errorf("пустое имя перебило настоящее: %q", got)
	}
	if got := noteAuthor(&Meeting{}); got != defaultNoteAuthor {
		t.Errorf("без участника ждали %q, получили %q", defaultNoteAuthor, got)
	}
}

func TestIsNote(t *testing.T) {
	if !isNote(&Meeting{MeetURL: noteURL}) {
		t.Error("заметку не узнали")
	}
	if isNote(&Meeting{MeetURL: "https://meet.google.com/abc-defg-hij"}) {
		t.Error("созвон приняли за заметку")
	}
	// Ссылка заметки не должна проходить за ссылку на созвон: иначе бот по ней
	// куда-нибудь пойдёт.
	if u := findMeetURL(noteURL); u != "" {
...
func testHub(t *testing.T, dir string) (*noteHub, *Config, *Store, chan string) {
	t.Helper()
	...
	h := &noteHub{
		record: func(path, device string, max time.Duration) (*Recorder, error) {
			...
			return &Recorder{Path: path, Started: time.Now()}, nil
		},
		process: func(_ context.Context, _ *Config, _ *Store, id string, _ bool) error {
			processed <- id
			return nil
		},
	}
...
func TestProcessNoteМолчаниеНеИдётВClaude(t *testing.T) {
	...
	m := &Meeting{ID: "n-empty", Title: "Заметка 10.09 14:35", MeetURL: noteURL,
...
func TestRenameMeetingТолькоУИмениПоУмолчанию(t *testing.T) {
	...
	defer st.Close()
	m := &Meeting{ID: "n2", Title: "старое", MeetURL: noteURL,
...
func testNotePanel(t *testing.T) (*httptest.Server, *Store, *noteHub, chan string) {
	t.Helper()
	t.Setenv("STENO_PANEL_PASSWORD", "тайна")
	dir := t.TempDir()
	h, cfg, st, processed := testHub(t, dir)
	notes = h // ручки ходят к общей заметке процесса
	t.Cleanup(func() { notes = &noteHub{} })
...
func TestNoteСторожЛовитОтвалившийсяМикрофон(t *testing.T) {
	...
	h.record = func(path, device string, max time.Duration) (*Recorder, error) {
		if err := os.WriteFile(path, make([]byte, noteMinBytes+1), 0o644); err != nil {
			return nil, err
		}
		return &Recorder{Path: path, Started: time.Now(), done: dead}, nil
	}
...
func TestNoteСтартЗакрываетНезакрытуюОтПрошлогоЗапуска(t *testing.T) {
	dir := t.TempDir()
	h, cfg, st, _ := testHub(t, dir)
	stale := &Meeting{ID: "старая", Title: "Заметка вчера", MeetURL: noteURL,
	...
	call := &Meeting{ID: "созвон", Title: "Планёрка",
