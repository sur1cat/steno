...
func uiTestModel(t *testing.T, seed func(*Store)) *uiModel {
	t.Helper()
...
func uiSeed(t *testing.T) func(*Store) {
	t.Helper()
	return func(st *Store) {
		...
		must(st.SaveSegments(uiTestMeeting, []Segment{
		...
		must(st.SaveFollowup(uiTestMeeting, "claude", &Followup{
			...
			Decisions:     []Decision{{What: "релиз в пятницу", Why: "миграция не успевает", At: 380}},
			OpenQuestions: []OpenQuestion{{Question: "кто дежурит в выходные", WaitingOn: "Участник А", At: 1890}},
			Risks:         []string{"Откат миграции не проверялся на объёме прода."},
			Timeline:      []TimelineItem{{At: 10, Title: "Релиз", Summary: "обсудили дату"}},
		}))

		must(st.AddItem(ProjectItem{ID: "T-0001", Project: "Платежи", Kind: KindTask,
			Text: "закончить миграцию схемы", Owner: "Участник Б", Due: "2026-09-11",
			OpenedIn: uiTestMeeting}))
		must(st.AddItem(ProjectItem{ID: "Q-0002", Project: "Платежи", Kind: KindQuestion,
			Text: "кто дежурит в выходные", Owner: "Участник А", OpenedIn: uiTestMeeting}))
		must(st.AddItem(ProjectItem{ID: "T-0003", Project: "Онбординг", Kind: KindTask,
			Text: "прототип первых экранов", Owner: "Участник Д", Due: "2026-09-20",
			OpenedIn: uiTestMeeting}))
		must(st.AddItem(ProjectItem{ID: "T-0004", Project: "Платежи", Kind: KindTask,
			Text: "старая задача", Owner: "Участник А", OpenedIn: uiTestMeeting}))
		must(st.CloseItem("T-0004", "done", "сделано раньше", uiTestMeeting))
	}
}

func itemStatus(t *testing.T, st *Store, id string) string {
	t.Helper()
...
func TestUITranscriptScrolls(t *testing.T) {
	m := uiTestModel(t, func(st *Store) {
		st.CreateMeeting(&Meeting{ID: "m1", Title: "Долгий", MeetURL: "u",
			StartedAt: time.Now(), Status: "transcribed"})
		var segs []Segment
		for i := 0; i < 200; i++ {
			segs = append(segs, Segment{Start: float64(i * 10), End: float64(i*10 + 9),
...
func TestUITaskFilters(t *testing.T) {
	...
	if m.itemFilter.Kind != KindQuestion {
...
func TestUISearchJumpsToFoundPlace(t *testing.T) {
	m := uiTestModel(t, func(st *Store) {
		st.CreateMeeting(&Meeting{ID: "long-1", Title: "Долгий созвон", MeetURL: "u",
			StartedAt: time.Now(), Status: "transcribed"})
		var segs []Segment
		for i := 0; i < 120; i++ {
			speaker := "Участник А"
			...
			segs = append(segs, Segment{Start: float64(i * 20), End: float64(i*20 + 19),
...
func TestUIProjectRenameOntoExistingRefuses(t *testing.T) {
	m := uiTestModel(t, uiSeed(t))
	if err := m.st.SaveProject(Project{Name: "Онбординг"}); err != nil {
		t.Fatal(err)
	}
	if err := m.reload(); err != nil {
		t.Fatal(err)
...
func TestUIBuildContextNeedsSources(t *testing.T) {
	...
	m.Update(uiContextDone{Project: "Платежи", Spend: Spend{USD: 0.02}})
	if m.building["Платежи"] {
...
func chanValues(t *testing.T, st *Store, key string) (bool, map[string]string) {
	t.Helper()
...
func TestUIChannelListFieldAddsAndRemovesOneByOne(t *testing.T) {
	...
	if got := chList(values["allowed_chats"]); strings.Join(got, "|") != "-100111|-100222|-100333" {
	...
	if got := chList(values["allowed_chats"]); strings.Join(got, "|") != "-100111|-100333" {
...
func TestUINarrowTerminalNeverOverflows(t *testing.T) {
	...
	m := uiTestModel(t, func(st *Store) {
		...
		st.SaveProject(Project{
			...
			Sources: []Source{{Kind: "path", Value: "/" + long}},
		})
		st.CreateMeeting(&Meeting{ID: "long-1", Title: long, MeetURL: "u",
			StartedAt: time.Now(), Status: "published"})
		st.SaveSegments("long-1", []Segment{{Speaker: long, Text: long}})
		st.AddItem(ProjectItem{ID: "T-9999", Project: long, Kind: KindTask,
...
// панели (web/app/src/lib/fmt.ts): один созвон не должен называться в терминале
// иначе, чем в браузере.
func TestUIStatusWordsAndAlarm(t *testing.T) {
	...
	m := uiTestModel(t, func(st *Store) {
		st.CreateMeeting(&Meeting{ID: "bad-1", Title: "Не разослан", MeetURL: "u",
...
func TestUIHighlightMarksFound(t *testing.T) {
	s := uiHighlight("до " + markStart + "миграции" + markEnd + " после")
...
func TestUIWideWindowKeepsTablesTight(t *testing.T) {
	...
	m := uiTestModel(t, func(st *Store) {
		uiSeed(t)(st)
		if err := st.AddItem(ProjectItem{ID: "T-7777", Project: "Платежи", Kind: KindTask,
...
func TestUIMainColumnFollowsContentNotWindow(t *testing.T) {
	...
	m2 := uiTestModel(t, func(st *Store) {
		uiSeed(t)(st)
		must := func(err error) {
			t.Helper()
			if err != nil {
		...
		must(st.AddItem(ProjectItem{ID: "T-9999", Project: "Платежи", Kind: KindTask,
...
func TestUINarrowColumnsKeepGap(t *testing.T) {
	m := uiTestModel(t, func(st *Store) {
		...
		must(st.AddItem(ProjectItem{ID: "T-8888", Project: "Платежи", Kind: KindQuestion,
