...
func (p *Panel) Name() string { return i18n.Tr("панель") }

func NewPanel(cfg *core.Config, st *core.Store, lg *log.Logger) (*Panel, error) {
	pass, err := core.Secret(cfg.Panel.PasswordEnv, i18n.Tr("панель"))
	if err != nil {
		return nil, err
	}
	sum := sha256.Sum256([]byte("steno-panel:" + pass))
	return &Panel{cfg: cfg, st: st, log: lg, key: sum[:], google: google.NewGoogleStates()}, nil
}
...
func (p *Panel) Run(ctx context.Context) error {
	...
	p.log.Printf(i18n.Tr("панель: слушаю %s"), addr)
...
func (p *Panel) handler() http.Handler {
	...
	if err != nil {
		...
		p.log.Printf(i18n.Tr("панель: бандл не читается: %v"), err)
		dist = distFS
	}
...
func (p *Panel) passwordOK(given string) bool {
	pass, err := core.Secret(p.cfg.Panel.PasswordEnv, i18n.Tr("панель"))
	if err != nil {
		return false
	}
