...
func (p *Panel) apiGuard(h http.HandlerFunc) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil || !p.valid(c.Value) {
			writeJSON(w, http.StatusUnauthorized, map[string]string{"error": i18n.Tr("нужен вход")})
			return
		}
		h(w, r)
	})
...
func (p *Panel) apiFail(w http.ResponseWriter, err error) {
	p.log.Printf(i18n.Tr("панель: %v"), err)
	writeJSON(w, http.StatusInternalServerError,
		map[string]string{"error": i18n.Tr("что-то сломалось, смотри лог сервиса")})
}
...
func (p *Panel) apiLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(http.MaxBytesReader(w, r.Body, 4096)).Decode(&body); err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": i18n.Tr("плохой запрос")})
		return
	}
	if !p.passwordOK(body.Password) {
		p.log.Printf(i18n.Tr("панель: неверный пароль с %s"), clientIP(r))
		writeJSON(w, http.StatusUnauthorized, map[string]string{"error": i18n.Tr("не тот пароль")})
		return
	}
...
func (p *Panel) apiMeeting(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeJSON(w, http.StatusNotFound, map[string]string{"error": i18n.Tr("нет такого созвона")})
		return
	}
...
func (p *Panel) apiDeleteMeeting(w http.ResponseWriter, r *http.Request) {
	...
	p.log.Printf(i18n.Tr("панель: созвон %s удалён; пунктов ушло %d, открылось заново %d"), id,
...
func (p *Panel) apiSaveChannel(w http.ResponseWriter, r *http.Request) {
	key := r.PathValue("key")
	def, ok := core.ChannelByKey(key)
	if !ok {
		writeJSON(w, http.StatusNotFound, map[string]string{"error": i18n.Tr("нет такого канала")})
		return
	}
	var body struct {
...
func (p *Panel) apiBrowse(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": i18n.Tr("в эту папку не пускают")})
		return
	}
...
func browseResolve(home, ask string) (dir, root string, err error) {
	...
	if err != nil {
		return "", "", errors.New(i18n.Tr("домашний каталог не читается"))
	}
	...
	if err != nil {
		return "", "", errors.New(i18n.Tr("нет такой папки"))
	}
	if !withinDir(root, dir) {
		return "", "", errors.New(i18n.Tr("наружу из домашнего каталога нельзя"))
	}
	return dir, root, nil
}
...
func GoogleStatus(cfg *core.Config) googleState {
	s := googleState{Ready: google.GoogleOAuthReady(cfg), Severity: googleInfo}
	if t, err := google.LoadGoogleToken(cfg); err == nil {
		...
		if missing := google.MissingScopes(t.Scopes, google.GoogleScopes); len(missing) > 0 {
			s.Why = i18n.Tr("Доступ выдан не весь: не хватает доступа к ") +
				strings.Join(google.HumanScopes(missing), i18n.Tr(" и ")) + i18n.Tr(". Подключи ещё раз.")
	...
	case google.CredentialsFile(cfg.Calendar.CredentialsFile, cfg.GoogleDocs.CredentialsFile) != "":
		s.Why = i18n.Tr("Доступ уже выдан ключом организации — подключать ничего не нужно.")
	case !s.Ready:
		...
		s.Why = i18n.Tr("Чтобы появилась кнопка, выполни в терминале `steno setup` ") +
			i18n.Tr("и выбери «По кнопке в панели» — это делается один раз.")
		s.Severity = GoogleWarn
	}
	return s
}
...
func (p *Panel) apiGoogleConnect(w http.ResponseWriter, r *http.Request) {
	oc, err := google.OauthConfig(p.cfg, p.googleRedirect(r))
	if err != nil {
		p.log.Printf(i18n.Tr("панель: подключение Google: %v"), err)
		writeJSON(w, http.StatusBadRequest, map[string]string{
			"error": i18n.Tr("вход по кнопке ещё не настроен — это делает тот, кто ставил steno")})
		return
	}
...
func (p *Panel) apiGoogleCallback(w http.ResponseWriter, r *http.Request) {
	q := r.URL.Query()
	if e := q.Get("error"); e != "" {
		p.log.Printf(i18n.Tr("панель: Google отказал: %s"), e)
		p.googleBack(w, r, i18n.Tr("Согласие не получено — steno остался без доступа"))
		return
	}
	...
	if !p.google.Take(q.Get("state")) {
		p.googleBack(w, r, i18n.Tr("Ссылка устарела — нажми «Подключить Google» ещё раз"))
		return
	}
	oc, err := google.OauthConfig(p.cfg, p.googleRedirect(r))
	if err != nil {
		p.log.Printf(i18n.Tr("панель: подключение Google: %v"), err)
		p.googleBack(w, r, i18n.Tr("Доступ к Google не настроен — выполни `steno setup` в терминале"))
		return
	}
	tok, err := oc.Exchange(r.Context(), q.Get("code"))
	if err != nil {
		p.log.Printf(i18n.Tr("панель: обмен кода Google: %v"), err)
		p.googleBack(w, r, i18n.Tr("Google не подтвердил согласие — попробуй ещё раз"))
		return
	}
	...
	if tok.RefreshToken == "" {
		old, oerr := google.LoadGoogleToken(p.cfg)
		if oerr != nil || old.Token.RefreshToken == "" {
			p.googleBack(w, r, i18n.Tr("Google выдал доступ на час вместо постоянного. ")+
				i18n.Tr("Убери steno из разрешённых приложений в своём аккаунте Google и подключи заново"))
			return
		}
		tok.RefreshToken = old.Token.RefreshToken
	}
	account, err := google.GoogleAccountEmail(r.Context(), oc, tok)
	if err != nil {
		p.log.Printf(i18n.Tr("панель: чей ящик подключили, узнать не вышло: %v"), err)
		p.googleBack(w, r, i18n.Tr("Не смог узнать, к какому ящику подключился — попробуй ещё раз"))
		return
	}
	if err := google.SaveGoogleToken(p.cfg, &google.GoogleToken{
		Account: account, Scopes: google.GoogleScopes, Token: tok,
	}); err != nil {
		p.log.Printf(i18n.Tr("панель: не сохранился доступ в Google: %v"), err)
		p.googleBack(w, r, i18n.Tr("Не смог сохранить доступ, смотри лог сервиса"))
		return
	}
	p.log.Printf(i18n.Tr("панель: Google подключён как %s"), account)
	p.googleBack(w, r, "")
}

func (p *Panel) apiGoogleDisconnect(w http.ResponseWriter, r *http.Request) {
	if err := google.ForgetGoogleToken(p.cfg); err != nil {
		p.apiFail(w, err)
		return
	}
	p.log.Print(i18n.Tr("панель: доступ в Google отключён"))
	writeJSON(w, http.StatusOK, map[string]bool{"ok": true})
}
...
func (p *Panel) apiInvite(w http.ResponseWriter, r *http.Request) {
	...
	if p.D == nil {
		writeJSON(w, http.StatusServiceUnavailable, map[string]string{
			"error": i18n.Tr("бота зовёт сервис, а он сейчас не запущен")})
		return
	}
	id, res, err := sources.InviteToCall(r.Context(), p.D, body.URL, body.Title, i18n.Tr("панель"))
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
	...
	case sources.Started:
		writeJSON(w, http.StatusOK, map[string]any{
			"status": "started", "meetingId": id, "message": i18n.Tr("иду на созвон")})
	case sources.Duplicate:
		writeJSON(w, http.StatusOK, map[string]any{
			"status": "duplicate", "message": i18n.Tr("на этот созвон уже иду")})
	case sources.NoCapacity:
		writeJSON(w, http.StatusOK, map[string]any{
			"status": "no_capacity",
			"message": i18n.Tr("сейчас пишу максимум созвонов сразу — освободится слот, ") +
				i18n.Tr("позови ещё раз")})
	default:
		writeJSON(w, http.StatusInternalServerError, map[string]string{
			"error": i18n.Tr("не смог записать созвон, смотри лог сервиса")})
	}
}
...
func (p *Panel) apiSaveProject(w http.ResponseWriter, r *http.Request) {
	...
	if name == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": i18n.Tr("у проекта должно быть название")})
		return
	}
	for _, s := range body.Sources {
		...
		default:
			writeJSON(w, http.StatusBadRequest,
				map[string]string{"error": i18n.Tr("непонятный источник: ") + s.Kind})
			return
		}
		if strings.TrimSpace(s.Value) == "" {
			writeJSON(w, http.StatusBadRequest,
				map[string]string{"error": i18n.Tr("у источника «") + s.Kind + i18n.Tr("» пустое значение")})
			return
		}
	}
	...
	if err := p.st.SaveProject(core.Project{
...
func (p *Panel) apiBuildContext(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeJSON(w, http.StatusNotFound, map[string]string{"error": i18n.Tr("нет такого проекта")})
		return
	}
...
func (p *Panel) apiCloseItem(w http.ResponseWriter, r *http.Request) {
	if err := p.st.CloseItem(r.PathValue("id"), "done", i18n.Tr("закрыто руками"), ""); err != nil {
		p.apiFail(w, err)
		return
	}
	writeJSON(w, http.StatusOK, map[string]bool{"ok": true})
...
func spaHandler(dist fs.FS) http.Handler {
	...
	index = bytes.Replace(index, []byte(`<html lang="en">`), []byte(`<html lang="`+i18n.UILang+`">`), 1)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if indexErr != nil {
			http.Error(w, i18n.Tr("панель не собрана: запусти make panel"), http.StatusInternalServerError)
			return
		}
