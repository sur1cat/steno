...
func TestPanelNeverEmitsUserHTML(t *testing.T) {
	...
	if err := st.CreateMeeting(&core.Meeting{ID: "x1", Title: evil,
		MeetURL:   "https://meet.google.com/abc-defg-hij",
		StartedAt: time.Now(), Status: "published"}); err != nil {
		t.Fatal(err)
	}
	if err := st.SaveSegments("x1", []core.Segment{{Start: 1, End: 4, Speaker: "Участник А",
		Text: `говорим про миграцию <img src=x onerror=alert(1)>`}}); err != nil {
		t.Fatal(err)
	}
	if err := st.SaveFollowup("x1", "t", &core.Followup{Title: evil, TLDR: []string{"перенос"}}); err != nil {
		t.Fatal(err)
	}
