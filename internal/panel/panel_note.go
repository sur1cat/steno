...
func noteState(s *note.NoteSession) map[string]any {
	if s == nil {
...
func (p *Panel) apiNoteState(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, http.StatusOK, noteState(note.Notes.Live()))
}
...
func (p *Panel) apiNoteDevices(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		writeJSON(w, http.StatusOK, map[string]any{
			"devices": []audio.MicDevice{}, "error": err.Error()})
		return
	}
...
func (p *Panel) apiNoteStart(w http.ResponseWriter, r *http.Request) {
	...
	s, err := note.Notes.Start(p.cfg, p.st, p.log, note.NoteOptions{
...
func (p *Panel) apiNoteStop(w http.ResponseWriter, r *http.Request) {
	s, err := note.Notes.Stop(r.Context(), p.cfg, p.st, p.log, false)
	if err != nil {
		if s == nil {
			writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
			return
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"seconds": int(s.Elapsed().Seconds()),
		"message": i18n.Tr("расшифровываю и разбираю"),
	})
}

func (p *Panel) apiNoteCancel(w http.ResponseWriter, r *http.Request) {
	s, err := note.Notes.Cancel(p.st, p.log)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{
		"id": s.ID, "status": "cancelled", "message": i18n.Tr("заметка выброшена")})
}
