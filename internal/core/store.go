...
func OpenStore(dir string) (*Store, error) {
	...
	if _, err := db.Exec(schema); err != nil {
		return nil, fmt.Errorf(i18n.Tr("схема: %w"), err)
	}
	if _, err := db.Exec(searchSchema); err != nil {
		return nil, fmt.Errorf(i18n.Tr("индекс поиска: %w"), err)
	}
	if _, err := db.Exec(scheduleSchema); err != nil {
		return nil, fmt.Errorf(i18n.Tr("схема расписания: %w"), err)
	}
	...
	for table, cols := range want {
		...
		for name, decl := range missing {
			if _, err := db.Exec(`ALTER TABLE ` + table + ` ADD COLUMN ` + name + ` ` + decl); err != nil {
				return fmt.Errorf(i18n.Tr("миграция %s.%s: %w"), table, name, err)
			}
		}
	}
	return nil
...
func (s *Store) SaveSpend(meetingID string, sp Spend) error {
	...
	if n, err := res.RowsAffected(); err == nil && n == 0 {
		return fmt.Errorf(i18n.Tr("нет follow-up для %s — расход некуда записать"), meetingID)
	}
	return nil
}
...
// EventAttempted — ходил ли бот на это событие хоть раз, чем бы дело ни
// кончилось. Нужно источникам, которые зовут сами и по кругу: календарь
// опрашивается каждые две минуты, и «созвон кончился — путь свободен» для
// него значило бы заход в пустую комнату каждым опросом до конца события.
// Так и случилось на живом прогоне: бот вышел «остался один», а через минуту
// пошёл снова, и записал тишину, и отдал её модели. Человек, зовущий через
// Telegram, зовёт осознанно — ему EventSeen; автомату — одна попытка.
func (s *Store) EventAttempted(key string) (bool, error) {
	var n int
...
func (s *Store) CountMeetingsSince(t time.Time) (int, error) {
	var n int
	err := s.DB.QueryRow(`SELECT COUNT(*) FROM meetings WHERE started_at >= ?`, t.Unix()).Scan(&n)
	return n, err
}
