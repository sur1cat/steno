...
func (t meetingToll) words() string {
	var gone []string
	if t.Bytes > 0 {
		gone = append(gone, i18n.Trf("запись (%s)", i18n.Trf("%.1f МБ", float64(t.Bytes)/(1<<20))))
	}
	if t.Segments > 0 {
		gone = append(gone, i18n.Trf("расшифровка (%s)",
			i18n.Plural(t.Segments, i18n.Tr("реплика"), i18n.Tr("реплики"), i18n.Tr("реплик"))))
	}
	if t.Followup {
		gone = append(gone, "follow-up")
	}
	if t.OpenedTasks > 0 {
		gone = append(gone, i18n.Plural(t.OpenedTasks, i18n.Tr("задача"), i18n.Tr("задачи"), i18n.Tr("задач")))
	}
	if t.OpenedDecisions > 0 {
		gone = append(gone, i18n.Plural(t.OpenedDecisions, i18n.Tr("решение"), i18n.Tr("решения"), i18n.Tr("решений")))
	}
	if t.OpenedQuestions > 0 {
		gone = append(gone, i18n.Plural(t.OpenedQuestions, i18n.Tr("вопрос"), i18n.Tr("вопроса"), i18n.Tr("вопросов")))
	}

	out := i18n.Tr("уйдёт только строка в списке")
	if len(gone) > 0 {
		out = i18n.Trf("уйдут: %s", strings.Join(gone, ", "))
	}
	if t.Reopen > 0 {
		out += i18n.Trf("; %s откроется заново", t.ReopenWords())
	}
	return out
}
...
func (t meetingToll) ReopenWords() string {
	return i18n.Plural(t.Reopen, i18n.Tr("пункт"), i18n.Tr("пункта"), i18n.Tr("пунктов"))
}
