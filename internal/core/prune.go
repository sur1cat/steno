	...
	if d := cfg.Retention.Recordings.D(); d > 0 {
		...
		for _, id := range ids {
			...
			if err := os.RemoveAll(dir); err != nil {
				lg.Printf(i18n.Tr("не удалил %s: %v"), dir, err)
				continue
			}
			...
			if _, err := st.DB.Exec(`UPDATE meetings SET audio_path='' WHERE id=?`, id); err != nil {
				lg.Printf(i18n.Tr("не обновил %s: %v"), id, err)
			}
			res.Recordings++
			res.Bytes += size
		}
	...
	if d := cfg.Retention.Events.D(); d > 0 {
		...
		if r, err := st.DB.Exec(`DELETE FROM seen_events WHERE meeting_id NOT IN
			...
			if n, _ := r.RowsAffected(); n > 0 {
				fmt.Printf(i18n.Tr("отметок без созвона: %d\n"), n)
			}
		}
...
func (r PruneResult) String() string {
	return fmt.Sprintf(i18n.Tr("удалено записей: %d (%.1f ГБ), отметок о событиях: %d"),
