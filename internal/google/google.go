...
func GoogleClient(ctx context.Context, cfg *core.Config, credFile, subject string, scopes ...string) (option.ClientOption, error) {
	...
	case err == nil:
		...
		if subject != "" && !strings.EqualFold(subject, t.Account) {
			return nil, fmt.Errorf(
				i18n.Tr("Google подключён как %s, а тут нужен доступ от имени %s — ")+
					i18n.Tr("по кнопке steno работает только от того, кто её нажал; ")+
					i18n.Tr("чужой календарь и чужую почту так не открыть"),
				t.Account, subject)
		}
	...
	if credFile == "" {
		return nil, errors.New(
			i18n.Tr("нет доступа в Google: либо нажми «Подключить Google» в настройках панели, ") +
				i18n.Tr("либо укажи ключ организации (google_docs.credentials_file) — ") +
				i18n.Tr("он нужен, чтобы читать чужие календари"))
	}
	raw, err := os.ReadFile(credFile)
	if err != nil {
		return nil, fmt.Errorf(i18n.Tr("ключ service-account: %w"), err)
	}
	jwt, err := google.JWTConfigFromJSON(raw, scopes...)
	if err != nil {
		return nil, fmt.Errorf(i18n.Tr("разбор ключа %s: %w"), credFile, err)
	}
	jwt.Subject = subject
