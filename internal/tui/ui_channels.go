...
func uiSaveChannel(st *core.Store, key string, enabled bool, fresh map[string]string) error {
	def, ok := core.ChannelByKey(key)
	if !ok {
		return fmt.Errorf(i18n.Tr("нет такого канала: %s"), key)
	}
...
func uiChannelWhat(ch core.Channel) string {
	...
	case ch.In:
		return i18n.Tr("приносит созвоны")
	case ch.Out:
		return i18n.Tr("шлёт follow-up")
	}
	return ""
}

func uiFilterChannels(rows []core.Channel, text string) []core.Channel {
	needle := strings.ToLower(strings.TrimSpace(text))
	if needle == "" {
		return rows
	}
	out := make([]core.Channel, 0, len(rows))
	for _, c := range rows {
