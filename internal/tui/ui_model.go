...
var uiTabTitles = sync.OnceValue(func() [tabCount]string {
	return [tabCount]string{i18n.Tr("Созвоны"), i18n.Tr("Задачи"), i18n.Tr("Проекты"), i18n.Tr("Поиск"), i18n.Tr("Каналы")}
})
...
func NewUIModel(cfg *core.Config, st *core.Store) *uiModel {
	return &uiModel{
		cfg: cfg, st: st,
		w: 80, h: 24,
...
func (m *uiModel) visibleMeetings() []core.MeetingRow {
	return uiFilterMeetings(m.meetings, m.filter[tabMeetings].String())
}

func (m *uiModel) visibleItems() []core.ProjectItem {
	f := m.itemFilter
...
func (m *uiModel) visibleChannels() []core.Channel {
...
func (m *uiModel) selectedMeeting() (core.MeetingRow, bool) {
	rows := m.visibleMeetings()
	if i := m.cursor[tabMeetings]; i >= 0 && i < len(rows) {
		return rows[i], true
	}
	return core.MeetingRow{}, false
}

func (m *uiModel) selectedItem() (core.ProjectItem, bool) {
	rows := m.visibleItems()
...
func (m *uiModel) selectedChannel() (core.Channel, bool) {
	rows := m.visibleChannels()
	if i := m.cursor[tabChannels]; i >= 0 && i < len(rows) {
		return rows[i], true
	}
	return core.Channel{}, false
}

func (m *uiModel) selectedHit() (core.SearchHit, bool) {
...
func (m *uiModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case uiContextDone:
		...
		if msg.Err != nil {
			m.fail(fmt.Errorf(i18n.Tr("справка «%s»: %v"), msg.Project, msg.Err))
		} else {
			m.say(i18n.Tr("справка «%s» собрана, %s"), msg.Project, msg.Spend)
...
func (m *uiModel) keyList(k string) tea.Cmd {
	...
	case "r":
		if err := m.Reload(); err != nil {
			...
			m.sayText(i18n.Tr("перечитал из базы"))
		}
		return nil
	case "esc":
		m.clearFilters()
...
func (m *uiModel) keyTasks(k string) tea.Cmd {
	...
	case "enter":
		...
		if it.OpenedIn == "" {
			m.fail(fmt.Errorf(i18n.Tr("у пункта %s не записан созвон, на котором он появился"), it.ID))
			return nil
		}
		m.openMeeting(it.OpenedIn, scrMeeting, -1)
	case "d":
		m.closeSelectedItem("done", i18n.Tr("закрыто из терминала"))
	case "x":
		m.closeSelectedItem("dropped", i18n.Tr("снято из терминала"))
	case "u":
		it, ok := m.selectedItem()
		if !ok {
			return nil
		...
		if it.Status == "open" {
			m.fail(fmt.Errorf(i18n.Tr("%s и так открыт"), it.ID))
			return nil
		}
	...
	case "c":
		...
		m.cursor[tabTasks] = 0
		m.clampCursor()
		m.sayText(i18n.Tr("фильтры сняты"))
	}
	return nil
}
...
func (m *uiModel) closeSelectedItem(status, note string) {
	...
	if it.Status != "open" {
		m.fail(fmt.Errorf(i18n.Tr("%s уже закрыт"), it.ID))
		return
	}
...
func (m *uiModel) openProjectPicker() {
	labels := []string{i18n.Tr("все проекты")}
	values := []string{""}
	for _, p := range m.projects {
		labels = append(labels, p.Name)
		values = append(values, p.Name)
	}
	m.showPicker(&uiPicker{title: i18n.Tr("Проект"), labels: labels, values: values,
		target: pickProject, current: m.itemFilter.Project})
}

func (m *uiModel) openOwnerPicker() {
	labels := []string{i18n.Tr("все исполнители")}
	values := []string{""}
	for _, o := range uiOwners(m.items) {
		labels = append(labels, o)
		values = append(values, o)
	}
	m.showPicker(&uiPicker{title: i18n.Tr("Исполнитель"), labels: labels, values: values,
		target: pickOwner, current: m.itemFilter.Owner})
}

func (m *uiModel) openKindPicker() {
	m.showPicker(&uiPicker{
		title:  i18n.Tr("Вид пункта"),
		labels: []string{i18n.Tr("всё"), i18n.Tr("задачи"), i18n.Tr("открытые вопросы"), i18n.Tr("решения")},
		values: []string{"", string(core.KindTask), string(core.KindQuestion), string(core.KindDecision)},
...
func (m *uiModel) keyPicker(k string) tea.Cmd {
	...
	case "enter", "right":
		...
		case pickOwner:
			...
			m.cursor[tabTasks] = 0
		case pickKind:
			m.itemFilter.Kind = core.ItemKind(v)
			m.cursor[tabTasks] = 0
		case pickSourceKind:
			if m.form != nil {
...
func (m *uiModel) askDeleteProject(p uiProjectRow) {
	if !p.Registered {
		m.fail(fmt.Errorf(i18n.Tr("«%s» в реестре нет — он только упомянут в задачах, удалять нечего"), p.Name))
		return
	}
	m.confirm = &uiConfirm{
		text:   fmt.Sprintf(i18n.Tr("Удалить проект «%s»? Задачи и решения по нему останутся."), p.Name),
...
func (m *uiModel) askDeleteMeeting(id, title string) {
	...
	m.confirm = &uiConfirm{
		text:   fmt.Sprintf(i18n.Tr("Удалить созвон «%s»? Навсегда, %s."), name, toll.Text),
...
func (m *uiModel) keyConfirm(k string) tea.Cmd {
	...
	case "y", "Y", "н", "Н":
		...
		case actDeleteProject:
			...
			m.clampCursor()
			m.say(i18n.Tr("проект «%s» удалён; задачи и решения по нему остались"), c.arg)
		case actDeleteMeeting:
			...
			m.clampCursor()
			if toll.Reopen > 0 {
				m.say(i18n.Tr("созвон удалён; вернулось в работу: %s"), toll.ReopenWords())
			} else {
...
func (m *uiModel) buildContext(p uiProjectRow) tea.Cmd {
	if len(p.Sources) == 0 {
		m.fail(fmt.Errorf(i18n.Tr("у «%s» нет источников — добавь репозиторий или каталог (e — правка)"), p.Name))
		return nil
	}
	if m.building[p.Name] {
		m.say(i18n.Tr("справка «%s» уже собирается"), p.Name)
		return nil
	}
	m.building[p.Name] = true
	m.say(i18n.Tr("собираю справку «%s» — это поход в Claude, займёт до минуты"), p.Name)
...
func (m *uiModel) openMeeting(id string, screen uiScreen, at float64) {
	...
	if err != nil {
		m.fail(fmt.Errorf(i18n.Tr("созвон %s: %v"), id, err))
		return
	}
	m.meeting = mt
...
func (m *uiModel) keyCard(k string) tea.Cmd {
	...
	case scrMeeting:
		...
		case "t":
			if len(m.segments) == 0 {
				m.fail(errors.New(i18n.Tr("расшифровки нет — созвон ещё не разобран")))
				return nil
			}
...
func (m *uiModel) keyForm(msg tea.KeyMsg) tea.Cmd {
	...
	case "shift+tab", "up":
		...
		return nil
	case "ctrl+n":
		m.showPicker(&uiPicker{title: i18n.Tr("Вид источника"),
			labels: []string{i18n.Tr("репозиторий (git-ссылка)"), i18n.Tr("каталог с кодом на этой машине"),
				i18n.Tr("сайт или документ"), i18n.Tr("просто текст")},
	...
	case "ctrl+k":
		if i := f.currentSource(); i >= 0 {
			...
			m.fail(errors.New(i18n.Tr("ctrl+k убирает источник — встань на строку источника")))
		}
		return nil
	case "ctrl+t":
		if i := f.currentSource(); i >= 0 {
			...
			m.fail(errors.New(i18n.Tr("ctrl+t меняет вид источника — встань на строку источника")))
		}
		return nil
	}
...
func (m *uiModel) afterChannelChange(key string, enabled bool) {
	...
	m.selectChannel(key)
	name := key
	if d, ok := core.ChannelByKey(key); ok {
		name = d.Name
	}
	word := i18n.Tr("выключен")
	if enabled {
		word = i18n.Tr("включён")
	}
	...
	tail := ""
	if d, ok := core.ChannelByKey(key); ok && !d.Live {
		tail = i18n.Tr("; применится после перезапуска serve")
	}
...
func (m *uiModel) keyChannelForm(msg tea.KeyMsg) tea.Cmd {
	...
	case "ctrl+n":
		...
		if field < 0 || f.fields[field].def.Kind != "list" {
			m.fail(errors.New(i18n.Tr("ctrl+n добавляет значение в список — встань на строку списка")))
			return nil
		}
		f.addItem(field)
		return nil
	case "ctrl+k":
		if !f.removeItem(row.field, row.item) {
			m.fail(errors.New(i18n.Tr("ctrl+k убирает значение списка — встань на такую строку")))
		}
		return nil
	}
...
func (m *uiModel) saveChannelForm() {
	...
	m.say(i18n.Tr("«%s» сохранён; сервис подхватит это после перезапуска serve"), name)
}
...
func (m *uiModel) saveForm() {
	...
	if name == "" {
		m.fail(errors.New(i18n.Tr("у проекта должно быть название")))
		f.field = uiFieldName
		return
	}
	...
	for i, s := range f.sources {
		...
		if v == "" {
			m.fail(fmt.Errorf(i18n.Tr("источник «%s» пустой — заполни или убери (ctrl+k)"),
		...
		sources = append(sources, core.Source{Kind: s.kind, Value: v})
	}
	p := core.Project{
		Name:       name,
		About:      strings.TrimSpace(f.about.String()),
		Aliases:    core.CommaList(f.aliases.String()),
		People:     core.CommaList(f.people.String()),
		Vocabulary: core.CommaList(f.words.String()),
		Sources:    sources,
	}
	...
	switch {
	case old == "":
		m.say(i18n.Tr("проект «%s» заведён"), name)
	case old != name:
		m.say(i18n.Tr("«%s» переименован в «%s» вместе с задачами"), old, name)
	default:
		m.say(i18n.Tr("проект «%s» сохранён"), name)
	}
}
