...
func (y CaptionYield) Report(speech time.Duration, wantLang string) string {
	...
	msg := fmt.Sprintf(i18n.Tr("субтитры: %d реплик, %d букв за %s (%.0f букв в минуту)"),
		y.Utterances, n, speech.Round(time.Second), rate)
	if s := y.Script(); s != "" {
		msg += i18n.Tr("; письменность — ") + i18n.ScriptName(s)
	}

	want := langScript[strings.ToLower(wantLang)]
	got := y.Script()
	if want != "" && got != "" && got != want {
		return msg + fmt.Sprintf(i18n.Tr(". Meet слушает не тот язык: просили %s (%s), ")+
			i18n.Tr("а текст идёт %s. Имена говорящих от этого верные, а текст субтитров ")+
			i18n.Tr("брать нельзя — язык распознавания ставится в самом Meet: ")+
			i18n.Tr("⋮ → Настройки → Субтитры → язык встречи"), wantLang, want, got)
	}
	if rate < sparseCaptions {
		return msg + i18n.Tr(". Это в разы меньше, чем речи: либо в звонке молчали, ") +
			i18n.Tr("либо Meet распознаёт не тот язык — проверь ⋮ → Настройки → Субтитры")
	}
	return msg
}
