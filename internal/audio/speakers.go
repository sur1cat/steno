...
func (c SpeakerCoverage) Report(elapsedSeconds float64) string {
	...
	if c.Spans == 0 {
		return i18n.Tr("подсветка говорящего: ни разу — имена придётся брать только из субтитров")
	}
	...
	for _, n := range names {
		parts = append(parts, fmt.Sprintf(i18n.Tr("%s — %.0f с"), n, c.People[n]))
	}
	return fmt.Sprintf(i18n.Tr("подсветка говорящего: %d отрезков, %.0f с речи из %.0f (%.0f%%); %s"),
