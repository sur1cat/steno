...
func StartRecording(path, source string) (*Recorder, error) {
	...
	if err := cmd.Start(); err != nil {
		logFile.Close()
		return nil, fmt.Errorf(i18n.Tr("ffmpeg: %w (установлен ли ffmpeg и поднят ли pulseaudio?)"), err)
	}
...
func (r *Recorder) Stop() error {
	...
	case <-time.After(15 * time.Second):
		...
		<-done
		waitErr = errors.New(i18n.Tr("не завершился за 15 с"))
	}
	...
	if waitErr != nil && !isInterrupted(waitErr) {
		return fmt.Errorf(i18n.Tr("ffmpeg оборвался (%w) — запись неполная, подробности в %s.ffmpeg.log"),
			waitErr, r.Path)
	}
	st, err := os.Stat(r.Path)
	if err != nil {
		return fmt.Errorf(i18n.Tr("запись не создана: %w"), err)
	}
	if st.Size() < 1024 {
		hint := r.emptyHint
		if hint == "" {
			hint = i18n.Tr("проверь PULSE_SINK и что Chromium играет в него")
		}
		return fmt.Errorf(i18n.Tr("запись пустая (%d байт) — %s"), st.Size(), hint)
	}
	...
	if got := oggDuration(r.Path); got > 0 {
		if want := r.Elapsed(); want > time.Minute && got < want/2 {
			return fmt.Errorf(i18n.Tr("в записи %s, а шла она %s — запись оборвалась, подробности в %s.ffmpeg.log"),
...
func StartMicRecording(path, device string, max time.Duration) (*Recorder, error) {
	if _, err := exec.LookPath("ffmpeg"); err != nil {
		return nil, fmt.Errorf(i18n.Tr("нужен ffmpeg, чтобы писать с микрофона: %w"), err)
	}
	...
	if err := cmd.Start(); err != nil {
		logFile.Close()
		return nil, fmt.Errorf(i18n.Tr("ffmpeg не запустился: %w"), err)
	}
	r := NewRecorder(cmd, logFile, path)
	r.emptyHint = i18n.Tr("похоже, микрофон молчал: проверь, то ли устройство выбрано и не выключен ли звук")
...
func (r *Recorder) startedOK(within time.Duration) error {
	...
	case <-r.Exited():
		if r.exitErr == nil {
			return errors.New(i18n.Tr("ffmpeg сразу закончил запись"))
		}
		return fmt.Errorf(i18n.Tr("запись не пошла: %w"), r.exitErr)
...
func micTrouble(logPath string) string {
	...
	if out, err := os.ReadFile(logPath); err == nil {
		if t := i18n.Tail(strings.TrimSpace(string(out)), 400); t != "" {
			b.WriteString("\n")
			b.WriteString(t)
		}
	}
	if runtime.GOOS == "darwin" {
		b.WriteString(i18n.Tr("\nЕсли микрофона тут быть не должно: macOS спрашивает разрешение у того ") +
			i18n.Tr("приложения, из которого запущен steno. Системные настройки → ") +
			i18n.Tr("Конфиденциальность и безопасность → Микрофон."))
	}
	return b.String()
}
...
func micInputArgs(device string) ([]string, error) {
	...
	case "windows":
		if d == "" {
			return nil, errors.New(i18n.Tr("на Windows нужно назвать микрофон: ") +
				i18n.Tr("ffmpeg -list_devices true -f dshow -i dummy покажет имена"))
		}
		return []string{"-f", "dshow", "-i", "audio=" + d}, nil
	}
	return nil, fmt.Errorf(i18n.Tr("не знаю, как писать микрофон на %s"), runtime.GOOS)
}
...
func MicDevices() ([]MicDevice, error) {
	if _, err := exec.LookPath("ffmpeg"); err != nil {
		return nil, fmt.Errorf(i18n.Tr("нужен ffmpeg: %w"), err)
	}
	if runtime.GOOS != "darwin" {
		...
		return nil, fmt.Errorf(i18n.Tr("список микрофонов steno умеет спрашивать только на macOS; ")+
			i18n.Tr("на %s назови устройство сам"), runtime.GOOS)
	}
	...
	if len(devs) == 0 {
		return nil, errors.New(i18n.Tr("ffmpeg не нашёл ни одного микрофона"))
	}
	return devs, nil
}
