...
func TestSyncSeesOnlyNewCommits(t *testing.T) {
	...
	src := core.Source{Kind: "path", Value: repo}
...
func TestSyncDoesNotTouchLocalWorkingCopy(t *testing.T) {
	...
	if _, err := syncRepo(context.Background(), t.TempDir(), "X",
		core.Source{Kind: "path", Value: repo}, st); err != nil {
		t.Fatal(err)
	}
...
func TestSyncOnNonRepoIsAnError(t *testing.T) {
	...
	defer st.Close()
	if _, err := syncRepo(context.Background(), t.TempDir(), "X",
		core.Source{Kind: "path", Value: t.TempDir()}, st); err == nil {
...
func TestPluralAgreement(t *testing.T) {
	for n, want := range map[int]string{
		...
		if got := i18n.Plural(n, "коммит", "коммита", "коммитов"); got != want {
			t.Errorf("i18n.Plural(%d) = %q, ожидали %q", n, got, want)
		}
	}
}
