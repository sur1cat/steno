// Пакет demo — правдоподобные созвоны, чтобы панель можно было посмотреть
// глазами до первого настоящего. Им пользуются `steno demo`, наполнение базы
// для картинок в README (cmd/steno/seed_test.go) и тесты MCP-сервера: те
// проверяют ответы на вопросы вроде «что решили про миграцию?», и задавать их
// выдуманной на месте базе из трёх строк значило бы проверять не то, что
// увидит человек.
//
// Отдельным пакетом, а не в cmd/steno: из package main ничего не импортировать.
package demo
...
// Seed наполняет базу демо-набором и возвращает проекты, которые к нему
// прилагаются, и число созвонов.
//
...
func Seed(st *core.Store, lang string) (projects []core.Project, n int, err error) {
	if lang == "en" {
		return seedEN(st)
	}
	return seedRU(st)
