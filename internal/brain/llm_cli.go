...
func ClaudeCLIAvailable() (bool, string) {
	if _, err := exec.LookPath("claude"); err != nil {
		return false, i18n.Tr("команда claude не найдена")
	}
	cmd := exec.Command("claude", "auth", "status")
	cmd.Env = scrubClaudeEnv(os.Environ())
	out, err := cmd.CombinedOutput()
	if err != nil {
		return false, "claude auth status: " + i18n.Tail(strings.TrimSpace(string(out)), 120)
	}
	...
	if err := json.Unmarshal(out, &st); err != nil {
		...
		return true, i18n.Tr("вход выполнен")
	}
	if !st.LoggedIn {
		return false, i18n.Tr("вход в Claude Code не выполнен — запусти `claude` и войди")
	}
	return true, core.FirstNonEmpty(st.AuthMethod, i18n.Tr("вход выполнен"))
}

// runClaudeCLI отправляет запрос и возвращает текст ответа.
func runClaudeCLI(ctx context.Context, cfg *core.Config, system, user string, maxUSD float64) (cliResult, error) {
	var res cliResult
	...
	if err != nil {
		...
		detail := ""
		if errors.As(err, &ee) {
			detail = i18n.Tail(strings.TrimSpace(string(ee.Stderr)), 400)
		}
	...
	if err := json.Unmarshal(out, &env); err != nil {
		return res, fmt.Errorf(i18n.Tr("claude -p вернул не JSON: %w\n%s"), err, i18n.Tail(string(out), 300))
	}
	if env.IsError {
		return res, fmt.Errorf("claude -p: %s", core.FirstNonEmpty(env.Subtype, i18n.Tail(env.Result, 300)))
	}
	if strings.TrimSpace(env.Result) == "" {
		return res, errors.New(i18n.Tr("claude -p вернул пустой ответ"))
	}
	res.Text = env.Result
	res.USD = env.Cost
	res.Usage = env.Usage
