...
func ClaudeClient(cfg *core.Config) (anthropic.Client, string, error) {
	key, err := core.Secret(cfg.Claude.APIKeyEnv, "Claude")
	if err == nil {
		return anthropic.NewClient(option.WithAPIKey(key)), i18n.Tr("ключ ") + cfg.Claude.APIKeyEnv, nil
	}
	...
	if !anthropicCredentialsAvailable() {
		return anthropic.Client{}, "", fmt.Errorf(
			i18n.Tr("нет доступа к Claude: переменная %s пуста, других учётных данных тоже не нашлось.\n")+
				i18n.Tr("  → ключ создаётся на console.anthropic.com → API keys\n")+
				"  → export %s=sk-ant-…", cfg.Claude.APIKeyEnv, cfg.Claude.APIKeyEnv)
	}
	return anthropic.NewClient(), i18n.Tr("профиль ant"), nil
}
