...
9. Расшифровка машинная: в ней есть оговорки, обрывы и неверно распознанные
   ...
   Пометка «(?)» сразу после слова означает, что распознавание расслышало это
   место плохо. Она поставлена не на глаз, а по числу, которое отдал сам движок
   распознавания. Читай её как «здесь звучало что-то другое», а не как «вот это
   слово неверное»: сомнение приходится на то место, где движок выбирал, а
   подставленное им слово нередко стоит следующим и выглядит уверенным.
   Помеченное — и то, что стоит вплотную к помеченному, — не факт.
   Пометка в конце всей реплики означает, что плохо расслышана вся она целиком.

   Что с этим делать:
   - Помеченное имя сверь со списком участников и «людей» проекта. Похоже
     ровно на одного — это он: назначай и вопроса не заводи, пометка уже
     сделала своё дело. Похоже на двоих или ни на кого — оставь как
     прозвучало и заведи открытый вопрос, что имя расслышано плохо, назвав
     кандидатов. Вопрос «подтвердите, что это Ануар», когда в проекте один
     Ануар и ни одного похожего имени, — лишняя работа человеку.
   - То же с названиями: помеченное слово, похожее ровно на одно из «сервисов
     и сокращений», — это оно, без вопроса. Похоже на несколько — вопрос.
   - Срок, расслышанный обрывком, пропадает совсем: «послезавтра» становится
     «и после», «к четвергу» становится «к чет». Увидев такой обрывок, не
     выбрасывай его: добавь в открытые вопросы, что срок назывался, но
     расслышан плохо, и приведи обрывок дословно.
   - Саму пометку в quote не переноси: её добавили мы, вслух её не произносили.

   Пометок может не быть вовсе: их отдаёт не всякий движок распознавания. Их
   отсутствие означает «неизвестно», а не «всё расслышано верно» — правила
   выше про машинные искажения действуют и без них.

   Срок, который стоит рядом с помеченным, но сам не помечен, оставь в due как
   есть: обрывок по соседству — не повод стирать то, что расслышано. К срокам,
   названным расплывчато вслух («потом», «как-нибудь»), это не относится — там
   терять нечего.
...
//
// Плохо расслышанные слова помечаются «(?)» прямо в тексте — здесь, а не в
// самой расшифровке. Разница принципиальная: сегменты в этом же виде уходят в
// базу, в панель и в опубликованный документ, и пометки там были бы мусором для
// человека. Модели же они нужны ровно там, где стоят: имя, на которое вешают
// задачу, помечено в том самом месте, где задачу дают (правило 9 системного
// промпта; сам знак — core.UncertainMark).
//
// Расшифровка, у которой уверенности нет (текст из субтитров Meet, движок без
// неё, расшифровка, поднятая из базы), проходит здесь без единой пометки — то
// есть ровно так, как проходила всегда.
func RenderTranscript(segs []core.Segment) string {
	var b strings.Builder
	last := ""
	for _, s := range segs {
		txt := core.MarkUncertain(s)
		if txt == "" {
			continue
		}
		sp := s.Speaker
