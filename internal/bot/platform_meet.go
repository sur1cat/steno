...
func (meetPlatform) SetCaptionLanguage(ctx context.Context, sel *Selectors, lang string, lg *log.Logger) {
	...
	for i := 0; i < captionSettingsTries && !opened; i++ {
		if err := chromedp.Run(ctx, chromedp.Evaluate(
			i18n.Tr(`window.__steno && window.__steno.captionSettingsStep
				? window.__steno.captionSettingsStep() : {state:"нет скрипта страницы"}`),
			&step)); err != nil {
			lg.Printf(i18n.Tr("настройки субтитров: %v"), err)
			return
		}
		if step.Done {
			opened = true
	...
	if !opened {
		...
		lg.Printf(i18n.Tr("не дошёл до настроек субтитров (остановился на %q%s) — Meet будет ")+
			i18n.Tr("слушать тем языком, который стоит у него сейчас; %s"),
			step.State, describeStep(step.Tabs, step.Buttons), i18n.Tr(manualLanguageHint))
	...
	if err := chromedp.Run(ctx, chromedp.Evaluate(js, &res)); err != nil {
		lg.Printf(i18n.Tr("выбор языка субтитров: %v"), err)
	} else if res.OK {
		lg.Printf(i18n.Tr("язык субтитров: %s (%s)"), res.Picked, res.How)
	} else {
		// Всегда говорим, чем кончилось: молчание здесь означало бы, что про
		// чужой язык человек узнает только по пустой расшифровке.
		lg.Printf(i18n.Tr("не нашёл %v среди языков субтитров; выпадашки: %v; варианты: %v; %s"),
			names, res.Combos, res.Options, i18n.Tr(manualLanguageHint))
	}
	...
	_ = chromedp.Run(ctx, chromedp.Evaluate(
		...
		&now))
	if now != "" {
		lg.Printf(i18n.Tr("Meet слушает языком: %s"), now)
	}
...
func describeStep(tabs, buttons []string) string {
	switch {
	case len(tabs) > 0:
		return fmt.Sprintf(i18n.Tr("; вкладки: %v"), tabs)
	case len(buttons) > 0:
		return fmt.Sprintf(i18n.Tr("; кнопки: %v"), buttons)
	}
	return ""
}
