...
//
// Субтитры площадки — главный источник имён говорящих: whisper слышит речь, но
// не знает, кто говорит, а имя в строке субтитров привязано к конкретной
// фразе. Поэтому «субтитров не будет» — это не поломка, но и не мелочь.
//
// Второй источник — подсветка говорящего (speakers.go). Она не зависит ни от
// языка распознавания, ни от того, включены ли субтитры, и на публичном Jitsi
// она единственная. Но она слабее: реагирует на громкость, а не на слова, — и
// потому достраивает субтитры, а не заменяет их.
...
func (c CaptionSupport) String() string {
	...
	case CaptionsOptional:
		return i18n.Tr("зависит от сервера")
	default:
		return i18n.Tr("нет")
	}
}
...
func (c CaptionState) Explain(p Platform) string {
	name := i18n.Tr("площадка")
	if p != nil {
		name = p.Title()
	}
	switch c {
	case CaptionsWorked:
		return i18n.Tr("субтитры сняты, имена говорящих есть")
	case CaptionsNone:
		return i18n.Tr("субтитров нет (") + name + i18n.Tr(" их не отдаёт) — расшифровка по звуку, ") +
			i18n.Tr("имена только от подсветки говорящего")
	default:
		return i18n.Tr("субтитры не включились, хотя должны были — расшифровка по звуку, ") +
			i18n.Tr("имена только от подсветки говорящего; похоже на смену вёрстки, ") +
			i18n.Tr("смотри selectors.json")
	}
}
...
var knownUnsupported = sync.OnceValue(func() []unsupportedSite {
	return []unsupportedSite{
		...
		{regexp.MustCompile(`(?i)(?:^|[^\w.-])telemost\.yandex\.[a-z]+/`), i18n.Tr("Яндекс Телемост")},
		{regexp.MustCompile(`(?i)(?:^|[^\w.-])ktalk\.ru/`), i18n.Tr("Контур.Толк")},
...
func LinkHint(text string) string {
	if meetLookupRe.MatchString(text) {
		return i18n.Tr("это ссылка вида meet.google.com/lookup — кода комнаты в ней нет; ") +
			i18n.Tr("нужна ссылка с кодом вида abc-defg-hij")
	}
	if name := unsupportedPlatform(text); name != "" {
		return name + i18n.Tr(" я пока не умею — работаю только с ") + SupportedPlatforms()
	}
	return ""
}
...
func MeetingLinkError(text string) error {
	...
	return fmt.Errorf(i18n.Tr("не нашёл ссылку на созвон — умею %s"), SupportedPlatforms())
}
...
func ApplyPlatformConfig(cfg *core.Config, lg *log.Logger) {
	sel, err := LoadSelectors(cfg.Bot.Selectors)
	if err != nil {
		lg.Printf(i18n.Tr("selectors: %v — беру встроенные"), err)
		return
	}
...
// FindMeetURL приводит любую форму ссылки к канонической. Возвращает пустую
// строку, если это не ссылка на поддержанную площадку: на этом держится вся
// защита от того, чтобы бота увели на чужую страницу.
//
// Само опознание живёт в platform.go — по одной реализации на площадку.
// Здесь остался вход для тех, кому площадка не нужна, а нужна только ссылка.
func FindMeetURL(text string) string {
	u, _ := findMeetingURL(text)
	return u
}
