...
type Dispatcher struct {
	...
	run func(ctx context.Context, cfg *Config, st *Store, m *Meeting) error
}

func newDispatcher(cfg *Config, st *Store, lg *log.Logger) *Dispatcher {
	n := cfg.Calendar.MaxConcurrent
	if n < 1 {
		n = 4
	}
	return &Dispatcher{cfg: cfg, st: st, log: lg, sem: make(chan struct{}, n),
		run: recordAndProcess}
}
...
func (d *Dispatcher) Start(ctx context.Context, key string, m *Meeting, why string) StartResult {
	...
	go func() {
		...
		d.log.Printf("«%s»: %v", orDash(m.Title), err)
