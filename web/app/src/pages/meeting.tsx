...
export function MeetingPage() {
  ...
  useEffect(() => {
    ...
    if (Number.isFinite(t) && t > 0) seek(t, false);
  ...
  return (
    <>
      <PageHead
        title={f?.title || m.title || "Без названия"}
        sub={
          <span className="inline-flex flex-wrap items-center gap-2">
            ...
            {m.leftReason && <span>вышел: {m.leftReason}</span>}
          </span>
        }
      />
        ...
        <div className="space-y-6">
                      ...
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1 text-sm">
                          ...
                          <span className="text-[var(--muted-foreground)]">
                            · ждём: {qq.waiting_on || "не определено"}
                          </span>
                        </div>
                      </div>
                    ))}
          ...
          {m.spend?.input > 0 && (
            <p className="text-sm text-[var(--muted-foreground)]">
              follow-up: вход {m.spend.input} токенов, выход {m.spend.output}
              {m.spend.usd > 0 && ` · $${m.spend.usd.toFixed(3)}`}
            </p>
          )}

          {links.length > 0 && (
            <Section title="Опубликовано">
