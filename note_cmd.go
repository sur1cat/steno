...
func cmdNote(ctx context.Context, args []string) error {
	...
	title := fs.String("title", "", tr("как назвать заметку (иначе название придумает Claude)"))
	author := fs.String("author", "", tr("чьи это задачи; иначе — имя из системы"))
	device := fs.String("device", envOr("STENO_MIC", ""),
		tr("микрофон: номер или имя из --devices"))
	list := fs.Bool("devices", false, tr("показать микрофоны и выйти"))
	maxDur := fs.Duration("max", noteMaxDefault, tr("потолок записи"))
	noPublish := fs.Bool("no-publish", false, tr("не рассылать, только разобрать"))
	...
	if len(rest) > 0 {
		return processNote(ctx, cfg, st, rest[0], *noPublish)
	}
	...
	fmt.Fprintf(os.Stderr, tr("● пишу с микрофона. Enter — остановить и разобрать. Заметка %s\n"), s.ID)
	waitForStop(ctx, s)
	...
	if _, err := notes.Stop(stopCtx, cfg, st, lg, true); err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf(tr("разбора заметки %s нет: %w"), s.ID, err)
	}
	fmt.Println()
...
func waitForStop(ctx context.Context, s *noteSession) {
	...
	for {
		...
		case <-limit.C:
			clearLine(live)
			fmt.Fprintf(os.Stderr, tr("потолок записи (%s) — останавливаю\n"), s.Until.Sub(s.Started))
			return
		case <-tick.C:
			if live {
				fmt.Fprintf(os.Stderr, "\r● %s  ", clock(s.Elapsed().Seconds()))
			}
		}
	}
}
...
func printMics() error {
	...
	fmt.Println(tr("микрофоны (номер — для --device):"))
	for _, d := range devs {
		fmt.Printf("  %s  %s\n", d.Index, d.Name)
	}
	fmt.Println(tr("  без --device берётся тот, что выбран в системе"))
	return nil
}
