...
// Потолок на одну заметку. Человек, забывший нажать «стоп», не должен ни
// заполнить диск, ни оплатить час расшифровки: по истечении записи она
// закрывается сама и разбирается как обычная — сказанное не теряется.
const noteMaxDefault = time.Hour
...
func isNote(m *Meeting) bool { return m != nil && m.MeetURL == noteURL }
...
type noteHub struct {
	...
	record  func(path, device string, max time.Duration) (*Recorder, error)
	process func(ctx context.Context, cfg *Config, st *Store, id string, noPublish bool) error
}

var notes = &noteHub{}

func (h *noteHub) recorder() func(string, string, time.Duration) (*Recorder, error) {
	if h.record != nil {
		return h.record
	}
...
func (h *noteHub) processor() func(context.Context, *Config, *Store, string, bool) error {
	if h.process != nil {
		return h.process
	}
	return processNote
...
func (h *noteHub) Start(cfg *Config, st *Store, lg *log.Logger, o NoteOptions) (*noteSession, error) {
	if cfg.Transcribe.Source == "captions" {
		return nil, errors.New(tr("сейчас текст берётся из субтитров Meet, а у заметки их нет: ") +
			tr("для записи с микрофона нужен whisper или Groq — поменяй это в настройках расшифровки"))
	}
	h.mu.Lock()
	defer h.mu.Unlock()
	if h.cur != nil {
		return nil, fmt.Errorf(tr("заметка уже пишется (%s) — сначала останови её"),
	...
	s.rec = rec
	h.cur = s
	lg.Printf(tr("заметка %s: пишу с микрофона, потолок %s"), s.ID, max)
...
func (h *noteHub) Park(st *Store, lg *log.Logger) {
	h.mu.Lock()
	s := h.cur
	h.cur = nil
	h.mu.Unlock()
	...
	_ = st.FinishMeeting(s.ID, time.Time{}, time.Now(), []string{s.Author},
		"recorded", "", tr("сервис остановился во время заметки"))
	...
	s.done()
	lg.Printf(tr("заметка %s: сервис останавливается — запись закрыл, разобрать её: steno note %s"),
		s.ID, s.ID)
}
...
func (h *noteHub) healStaleNotes(st *Store, lg *log.Logger) {
	...
	for _, id := range ids {
		_ = st.FinishMeeting(id, time.Time{}, time.Now(), nil,
			"failed", tr("сервис остановился во время заметки"), noteLeftReason)
		lg.Printf(tr("заметка %s осталась незакрытой от прошлого запуска — разобрать её: steno note %s"),
			id, id)
	}
}
...
func (h *noteHub) watch(cfg *Config, st *Store, lg *log.Logger, s *noteSession, max time.Duration) {
	t := time.NewTimer(max)
	defer t.Stop()
	select {
	case <-t.C:
		...
		lg.Printf(tr("заметка %s: прошёл потолок %s — останавливаю сама"), s.ID, max)
	case <-s.rec.Exited():
		if h.Live() != s {
			return // остановили обычным способом, ffmpeg вышел по нашему SIGINT
		}
		lg.Printf(tr("заметка %s: микрофон отвалился на %s — разбираю записанное"),
			s.ID, s.Elapsed().Round(time.Second))
	}
	if _, err := h.Stop(context.Background(), cfg, st, lg, false); err != nil {
		lg.Printf(tr("заметка %s: %v"), s.ID, err)
	}
}
...
// браузера незачем.
func (h *noteHub) Stop(ctx context.Context, cfg *Config, st *Store, lg *log.Logger, wait bool) (*noteSession, error) {
	h.mu.Lock()
	s := h.cur
	h.cur = nil
	h.mu.Unlock()
	if s == nil {
		return nil, errors.New(tr("заметка сейчас не пишется"))
	}
	...
	if stopErr != nil {
		lg.Printf(tr("заметка %s: %v"), s.ID, stopErr)
		...
		reason = tr("запись оборвалась: ") + stopErr.Error()
	}
	if err := st.FinishMeeting(s.ID, time.Time{}, ended, []string{s.Author},
		"recorded", "", reason); err != nil {
		return s, err
	}
	lg.Printf(tr("заметка %s: записано %s"), s.ID, s.Elapsed().Round(time.Second))
...
func (h *noteHub) Cancel(st *Store, lg *log.Logger) (*noteSession, error) {
	h.mu.Lock()
	s := h.cur
	h.cur = nil
	h.mu.Unlock()
	...
	lg.Printf(tr("заметка %s: отменена, запись удалена"), s.ID)
	return s, nil
}
...
func processNote(ctx context.Context, cfg *Config, st *Store, id string, noPublish bool) error {
	m, err := st.Meeting(id)
	if err != nil {
		return fmt.Errorf(tr("заметка %s: %w"), id, err)
	}
	if m.AudioPath == "" {
		return fmt.Errorf(tr("запись заметки %s удалена по сроку хранения — расшифровывать нечего"), id)
	}
	if _, err := os.Stat(m.AudioPath); err != nil {
		return fmt.Errorf(tr("запись %s недоступна: %w"), m.AudioPath, err)
	}

	log.Printf(tr("расшифровываю заметку %s"), m.AudioPath)
	segs, notesTxt, err := runTranscriber(ctx, cfg, m.AudioPath,
		meetingVocabulary(ctx, cfg, st, m))
	if err != nil {
		_ = st.SetStatus(id, "failed", err.Error())
		return err
	}
	for _, line := range lastLines(notesTxt, 4) {
		log.Printf("  %s", line)
	}
	...
	if len(segs) == 0 {
		err := errors.New(tr("в заметке не разобрано ни слова — проверь, тот ли микрофон выбран и не выключен ли звук"))
		_ = st.SetStatus(id, "failed", err.Error())
		return err
	}
	...
	log.Printf(tr("реплик в заметке: %d"), len(segs))
	...
	if t := strings.TrimSpace(f.Title); t != "" && isDefaultNoteTitle(m.Title) {
		if err := st.SetTitle(id, t); err != nil {
			log.Printf(tr("не переименовал заметку: %v"), err)
		} else {
			m.Title = t
		}
	}
	...
	if len(errs) > 0 {
		...
		return fmt.Errorf(tr("заметка разобрана, но не разослана: %s"), msg)
	}
