...
func TestUILook(t *testing.T) {
	...
	one := func(st *Store) {
		...
		st.CreateMeeting(&Meeting{ID: "2026-09-09-1852-c3d4", Title: "Проверка",
		...
		st.SaveFollowup("2026-09-09-1852-c3d4", "claude", &Followup{
			Title: "Короткая проверка", TLDR: []string{"Записали пробный созвон."},
			ActionItems: []ActionItem{{Owner: "Rustem", What: "проверить рассылку", At: 30}},
		})
		st.AddItem(ProjectItem{ID: "T-0001", Kind: KindTask, Text: "проверить рассылку",
