...
func TestGatherSourcesReadsCommits(t *testing.T) {
	...
	material, _, err := gatherSources(context.Background(), t.TempDir(),
		Project{Name: "Платежи", Sources: []Source{{Kind: "path", Value: repo}}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestFingerprintTracksSources(t *testing.T) {
	dir := t.TempDir()
	a := Project{Name: "x", Sources: []Source{{Kind: "text", Value: "первое"}}}
	b := Project{Name: "x", Sources: []Source{{Kind: "text", Value: "второе"}}}
...
func TestGatherSourcesRejectsUnknownKind(t *testing.T) {
	_, _, err := gatherSources(context.Background(), t.TempDir(),
		Project{Name: "x", Sources: []Source{{Kind: "магия", Value: "?"}}})
	if err == nil {
