...
func gatherSources(ctx context.Context, dataDir string, p Project) (string, string, error) {
	...
	for _, src := range p.Sources {
		...
		default:
			return "", "", fmt.Errorf(tr("непонятный источник %q"), src.Kind)
		}
	}
...
func describeRepo(ctx context.Context, dir string) (string, string, error) {
	if st, err := os.Stat(dir); err != nil || !st.IsDir() {
		return "", "", errors.New(tr("не каталог"))
	}
	var b strings.Builder
	fmt.Fprintf(&b, tr("\n## Репозиторий %s\n"), filepath.Base(dir))
...
func fetchText(ctx context.Context, url string) (string, error) {
	...
	if resp.StatusCode != 200 {
		return "", fmt.Errorf(tr("код %d"), resp.StatusCode)
	}
...
func buildPrimer(ctx context.Context, cfg *Config, p Project, material string) (string, Spend, error) {
	if strings.TrimSpace(material) == "" {
		return "", Spend{}, errors.New(tr("нечего читать: не задан ни один источник"))
	}
	head := fmt.Sprintf(tr("Проект: %s\n"), p.Name)
	if len(p.Aliases) > 0 {
		head += fmt.Sprintf(tr("Как называют вслух: %s\n"), strings.Join(p.Aliases, ", "))
	}
	if p.About != "" {
		head += fmt.Sprintf(tr("Коротко: %s\n"), p.About)
	}

	// Справка собирается редко и читается на каждом созвоне — экономить на ней
	// не стоит, но и рассуждать тут особо не о чем.
	text, spend, err := askLLM(ctx, cfg, primerSystem, head+tr("\nМатериал:\n")+material, nil, 4000)
	if err != nil {
		return "", Spend{}, err
	}
	if text = strings.TrimSpace(text); text == "" {
		return "", Spend{}, errors.New(tr("вернулась пустая справка"))
	}
...
func renderPrimers(st *Store, projects []Project) string {
	...
	return tr("Справки о проектах — по ним видно, какими словами команда о них говорит:\n") + b.String()
}
