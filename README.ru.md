...
одной встречи. Задачу можно отдать агенту — ТЗ по ней пишется по твоему
репозиторию, работа идёт в отдельной рабочей копии, назад приходит ветка.
Один бинарник на Go — панель вшита в него, ставить на сервер
...
Задача с созвона — одна фраза; остальные девять десятых лежат в коде.
`steno spec` читает репозиторий проекта и пишет ТЗ: что известно, где это в
коде, что сделать, чем проверить — и, в разделе, который нельзя пропустить,
чего не хватает. Задачу не про код — «спросить у Вики про макет» — не берёт и
говорит почему, а не раздувает в документ, которого никто не просил.

<div align="center">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/term-spec-ru-dark.svg">
  <img src="assets/term-spec-ru-light.svg" width="869" alt="steno spec превращает задачу в ТЗ с местами в коде, шагами, проверками и списком того, чего не хватает; steno spec run отдаёт его агенту в отдельной рабочей копии, назад приходит ветка">
</picture>
</div>

`steno spec run` отдаёт ТЗ Claude Code или Codex — тому, кто пишет твои
follow-up, или `agent.provider`, чтобы выбрать, — в отдельной рабочей копии git
и на своей ветке. Назад приходит ветка, которую можно посмотреть. Никакого push
и никаких коммитов в основную.

По умолчанию выключено: `steno agent on` — или переключатель в строке меню —
это момент, когда ты даёшь steno право писать файлы и выполнять команды на этой
машине. И запускается только по действию человека — никогда из Telegram, почты
или HTTP-входа: на созвоне кто угодно может сказать «снеси репозиторий», и это
доедет задачей. Само ТЗ может писаться без спроса: `steno agent auto on`, и
каждая задача, легшая в проект, получает его сразу после созвона — кнопке в
панели, строке меню и `steno ui` остаётся только «отдать».
...
Follow-up рассылается, задачи из него остаются.

| | |
|---|---|
| 📄 **Google Docs** | документ на созвон — и один на проект, который переписывается на месте: одна ссылка всегда показывает текущее состояние |
| 💬 **Slack** | follow-up в канал и личное сообщение тому, на ком задача, — по имени |
| ✈️ **Telegram** | чат команды — и ответ туда, откуда бота позвали |
| 📜 **Свой адаптер** | Discord, Mattermost, Notion, Jira, вебхук, письмо — [короткий скрипт на каждый](#принеси-своё) |

Задачи, решения и открытые вопросы живут по проектам — в панели, `steno ui` и
строке меню, — пока что-нибудь их не закроет: следующий созвон, кнопка или
коммит. Раз в сутки новые коммиты сверяются с открытыми задачами, и сделанное
молча закрывается с коммитом в качестве доказательства.
...
неработающая площадка хуже отсутствующей: узнаешь об этом на созвоне. Для них
следующий шаг — запись с твоего ноутбука: микрофон плюс системный звук, без
бота вовсе.
