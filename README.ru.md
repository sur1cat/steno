...
## Чем платить за Claude

Вопрос задаётся при `steno setup`, и ответ зависит не от цены, а от того, кто
запускает процесс.

**Подписка, которая уже есть.** steno зовёт `claude -p` — штатный
неинтерактивный режим Claude Code, — и follow-up уходит в тот тариф, за который
уже заплачено. Ни ключа, ни второго счёта. Это установка «для себя»: свои
созвоны на своём ноутбуке.

```
claude auth login     # если ещё не входил
steno setup           # выбрать «Подписка Claude»
```

**Ключ API.** Нужен на сервере, и дело не в деньгах: `claude -p` требует входа,
который выполняет человек, а на сервер никто не заходит.

```
export ANTHROPIC_API_KEY=sk-ant-...
```

Что именно нашлось — скажет `steno doctor`. Прибить выбор гвоздями можно полем
`claude.via`: `cli`, `api` или `auto` (по умолчанию — есть ключ, берём ключ,
иначе смотрим на подписку).

Честная оговорка про подписку: Claude Code добавляет к каждому запросу
собственный системный промпт, поэтому в токенах follow-up выходит дороже, чем
тот же запрос через API. Платится это из тарифа, а не с карты, ради чего всё и
затевалось, — а `steno cost` в обоих случаях показывает потраченное на самом
деле, а не оценку.
