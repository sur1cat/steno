...
По умолчанию выключено: `"agent": {"enabled": true}` в `steno.json` — момент,
когда ты даёшь steno право писать файлы и выполнять команды на этой машине. И
запускается только по действию человека — никогда из Telegram, почты или
HTTP-входа: на созвоне кто угодно может сказать «снеси репозиторий», и это
доедет задачей.
