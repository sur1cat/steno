...
```
make build       # соберёт web/app, потом бинарник
make panel       # только фронт
make panel-dev   # vite на 5273, /api проксируется в запущенный steno serve
```

`make build` не падает без node, если `web/dist` уже собран, — но собрать
бинарник с пустой панелью молча не даст: «панель не открывается» потом
разбирают часами. Сам бандл в git не хранится: это производное от исходников,
а конфликты в минифицированном JS никому не нужны.
