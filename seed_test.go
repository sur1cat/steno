...
func TestSeed(t *testing.T) {
	...
	all := []demo{{
		...
		people: []string{"Аня Смирнова", "Боря Ким", "Вика Орлова", "Дима Пак"},
		f: &Followup{
			...
			ActionItems: []ActionItem{
				{Owner: "Боря Ким", What: "закончить миграцию схемы и прогнать её на стейджинге",
					Due: "2026-09-11", At: 412, Quote: "я закончу миграцию к четвергу, в пятницу утром прогоню на стейдже"},
				{Owner: "Вика Орлова", What: "обновить changelog и письмо клиентам под новую дату",
				...
				{Owner: "Дима Пак", What: "проверить, что откат миграции отрабатывает на копии продовой базы",
			...
			OpenQuestions: []OpenQuestion{
				{Question: "кто дежурит в выходные после выката", WaitingOn: "Аня Смирнова", At: 1890},
		...
		lines: []line{
			{"Аня Смирнова", "Так, давайте начнём с релиза. Боря, что с миграцией?", 12},
			{"Боря Ким", "Миграция готова процентов на восемьдесят, но я не успею прогнать её на стейджинге до среды.", 31},
			{"Аня Смирнова", "То есть в среду мы не выкатываемся.", 58},
			{"Боря Ким", "Не выкатываемся. Либо режем миграцию из релиза, либо двигаем дату.", 66},
			{"Вика Орлова", "Резать нельзя, половина фич в бете у трёх клиентов и они этого ждут.", 94},
			{"Дима Пак", "Плюс если резать, придётся откатывать половину веток, это ещё день.", 120},
			{"Аня Смирнова", "Значит двигаем на пятницу. Возражения?", 370},
			{"Боря Ким", "Я закончу миграцию к четвергу, в пятницу утром прогоню на стейдже.", 412},
			{"Дима Пак", "Я возьму копию прода и проверю откат. Никто ведь ни разу не проверял на таком объёме.", 760},
			{"Вика Орлова", "Переписать письмо — это полчаса, сделаю завтра.", 1105},
			{"Аня Смирнова", "Давайте про дежурство отдельно, сейчас не решим.", 1890},
			{"Дима Пак", "Ещё вопрос — нужен ли отдельный прогон нагрузочных после миграции?", 2210},
		},
	}, {
		...
		people: []string{"Аня Смирнова", "Дима Пак", "Егор Лисицын"},
		f: &Followup{
			...
			ActionItems: []ActionItem{
				{Owner: "Дима Пак", What: "добавить потолок ретраев и экспоненциальную паузу",
					Due: "2026-09-12", At: 900, Quote: "надо просто поставить максимум пять попыток и бэкофф"},
				{Owner: "Егор Лисицын", What: "завести алерт на глубину очереди",
					Due: "2026-09-09", At: 1500, Quote: "алерт на глубину очереди сделаю на этой неделе"},
			},
			Decisions: []Decision{{What: "потолок ретраев — пять попыток", Why: "дальше задача всё равно не проходит", At: 940}},
			Risks:     []string{"Пока алерта нет, следующий такой инцидент заметим так же — по жалобам."},
		},
		lines: []line{
			{"Аня Смирнова", "Давайте разберём, что вчера случилось с очередью.", 8},
			{"Дима Пак", "Воркеры ретраили упавшие задачи бесконечно и забили пул соединений к базе.", 40},
			{"Егор Лисицын", "Мы это заметили только когда клиенты начали писать.", 180},
			{"Дима Пак", "Надо просто поставить максимум пять попыток и бэкофф.", 900},
			{"Егор Лисицын", "Алерт на глубину очереди сделаю на этой неделе.", 1500},
		},
	}, {
		...
		people: []string{"Вика Орлова", "Ася Гринёва"},
		f: &Followup{
			...
			ActionItems: []ActionItem{
				{Owner: "Ася Гринёва", What: "собрать прототип трёхшагового онбординга",
					Due: "2026-09-15", At: 300, Quote: "прототип соберу к середине следующей недели"},
			},
		},
		lines: []line{
			{"Вика Орлова", "Пять экранов — это много, люди отваливаются на третьем.", 20},
			{"Ася Гринёва", "Прототип соберу к середине следующей недели.", 300},
		},
	}, {
		id: "2026-09-08-1400-g7h8", title: "Звонок с подрядчиком",
		ago: 40 * time.Minute, status: "recording",
		people: []string{"Аня Смирнова"},
	}}
	...
	first := &Followup{
		ActionItems: []ActionItem{
			{Owner: "Боря Ким", What: "закончить миграцию схемы", Due: "2026-09-11",
				Project: "Платежи", Quote: "я закончу миграцию к четвергу"},
			{Owner: "Дима Пак", What: "проверить откат на копии прода", Due: "2026-09-04",
				Project: "Платежи", Quote: "я возьму копию прода"},
			{Owner: "Дима Пак", What: "потолок ретраев и бэкофф", Due: "2026-09-12",
				Project: "инфра", Quote: "максимум пять попыток"},
			{Owner: "Ася Гринёва", What: "прототип трёхшагового онбординга", Due: "2026-09-15",
				Project: "регистрация"},
		},
		Decisions: []Decision{
		...
		OpenQuestions: []OpenQuestion{
			{Question: "кто дежурит в выходные после выката", WaitingOn: "Аня Смирнова", Project: "Платежи"},
	...
	if open, err := st.OpenItems("Платежи"); err == nil {
		for _, it := range open {
			if it.Kind == KindTask && it.Owner == "Дима Пак" {
